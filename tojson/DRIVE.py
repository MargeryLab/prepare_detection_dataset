...
ROOT_DIR = './'
DATA_DIR = '/media/margery/4ABB9B07DF30B9DB/MedicalImagingDataset/DRIVE/training/images'
ANNOTATION_TUMOR_DIR = '/media/margery/4ABB9B07DF30B9DB/MedicalImagingDataset/DRIVE/training/1st_manual'

INFO = {
    "description": "DRIVE Segmentation Dataset",
...
def filter_for_annotations(root, files, image_filename):
    ...
    file_name_prefix = basename_no_extension[:2]
    files = [os.path.join(root, f) for f in files]
    files = [f for f in files if re.match(file_types, f)]
    files = [f for f in files if re.match(file_name_prefix, os.path.splitext(os.path.basename(f))[0][:2])]
    return files
